...
build_rpm_in_chroot_no_install () {
    ...
    specPath=$(find $SPECROOT -name "$1.spec" -print -quit)
    specDir=$(dirname $specPath)
    rpmMacros=("${SHARED_RPM_MACROS[@]}" -D "_sourcedir $specDir" --undefine=_debugsource_packages)
...
cp -v $SPECROOT/edge-rpm-macros/macros $LFS/usr/etc/rpm/macros
mkdir -pv $LFS/usr/lib/rpm/azl
cp -v $SPECROOT/edge-rpm-macros/gen-ld-script.sh $LFS/usr/lib/rpm/azl/gen-ld-script.sh
cp -v $SPECROOT/edge-rpm-macros/generate-package-note.py $LFS/usr/lib/rpm/azl/generate-package-note.py
cp -v $SPECROOT/edge-rpm-macros/verify-package-notes.sh $LFS/usr/lib/rpm/azl/verify-package-notes.sh
...
MSOPENJDK_URL="https://files-rs.edgeorchestration.intel.com/files-edge-orch/microvisor/rpms/26.06/base/Packages/m/$MSOPENJDK_FILENAME"
case $(uname -m) in
    x86_64)  MSOPENJDK_EXPECTED_HASH="973e9a7c652ff4967575189687096db6441a355dfdfa9cec90933476f0bc2db5" ;;
...
build_rpm_in_chroot_no_install edge-rpm-macros
chroot_and_install_rpms edge-rpm-macros
chroot_and_install_rpms edge-check-macros
...
build_rpm_in_chroot_no_install edge-repos
