...
func packSingleSPEC(ctx context.Context, specFile, srpmFile, signaturesFile, buildDir, outDir, distTag string, srcConfig sourceRetrievalConfiguration, netOpsSemaphore chan struct{}) (outputPath string, err error) {
	...
	// Vendor and Distribution tag in spec file will take precedence over the injected values,
	// so we will remove them in srpmSpecFile to allow them to be injected during build time.
	spec, _ := os.Open(specFile)
	srpmSpec, _ := os.Create(srpmSpecFile)
	specScanner := bufio.NewScanner(spec)
	srpmSpecWriter := bufio.NewWriter(srpmSpec)
	for specScanner.Scan() {
		line := specScanner.Text()
		if !strings.HasPrefix(line, "Vendor:") && !strings.HasPrefix(line, "Distribution:") {
			srpmSpecWriter.WriteString(line + "\n")
		}
	}
	srpmSpecWriter.Flush()
	srpmSpec.Close()
	spec.Close()
	...
	err = rpm.GenerateSRPMFromSPEC(srpmSpecFile, workingDir, defines)
	if err != nil {
		return
	}
