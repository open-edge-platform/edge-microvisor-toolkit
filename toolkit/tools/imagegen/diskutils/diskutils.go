...
func CreatePartitions(diskDevPath string, disk configuration.Disk, rootEncryption configuration.RootEncryption,
	diskKnownToBeEmpty bool, EMTEncryptionEnabled bool,
	...
	for idx, partition := range disk.Partitions {
		...
		// Full Disk Encryption and DM Verity requires a unique UUID of root partition from each build,
		// So to make sure, generate new random uuid and set it to root partition.
		if EMTEncryptionEnabled && partition.ID == "rootfs" {

			// tune2fs requires you to run 'e2fsck -f' first.
			err := shell.ExecuteLive(true /*squashErrors*/, "e2fsck", "-fy", partDevPath)
			if err != nil {
			...
			logger.Log.Debugf("Setting new UUID: %s, to partition device path: %v", newUuid, partDevPath)
			err = shell.ExecuteLive(true /*squashErrors*/, "tune2fs", "-U", newUuid, partDevPath)
			if err != nil {
				err = fmt.Errorf("failed to set new uuid with tune2fs:\n%w", err)
				return partDevPathMap, partIDToFsTypeMap, encryptedRoot, err
			}
		}
