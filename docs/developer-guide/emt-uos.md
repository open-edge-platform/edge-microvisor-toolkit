...
```bash
sudo make image -j8 REBUILD_TOOLS=y REBUILD_PACKAGES=n CONFIG_FILE=./imageconfigs/edge-image-tink.json
```

The build results in a compressed `emt-tink.tar.gz` file.

The `initramfs` and `vmlinuz` µOS images are required to run entirely in RAM memory, so first
...
* The `<emt-tink.tar.gz>` is the output "rootfs.tar.gz" file generated by uOS build.
