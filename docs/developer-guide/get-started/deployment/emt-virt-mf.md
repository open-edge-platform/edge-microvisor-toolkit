...
This article will guide you through setup and configuration of Virtual Machine Guest OS on Ubuntu 22.04 Host OS. The four supported OS are Anroid, Yocto, Windows 10 and Ubuntu.

## Android

This section describes setup, running and configuration of an Android VM using EMT Virt.
...
- Around 350 GB of free disk space on your workstation is required to store the source code and build artifacts.
...
Users of Celadon-in-VM (CIV) release must ensure that Celadon platform host OS hardening measures are in place to provide a secure computing base. This is essential to ensuring CIV security can be trusted in CIV operations.
...
Create an Android CIV image for running as VM on the host device:
...
For Android 12 Guest VM edit the `memory` and `vcpu` sections of the `/home/$USER/.intel/.civ/Android-CIV1.ini` configuration file.
...
This section describes setup, running and configuration of a Yocto VM using EMT Virt.
...
For Yocto guest VMs, PCI Wi-Fi device passthrough can be setup by adding the `--passthrough-pci-wifi` parameter to the guest VM launch command:
...
This section describes setup, running and configuration of a Windows 10 VM using EMT Virt.
...
1. Passthrough of all USB Host Devices
   ...
   USB host passthrough parameter can be added in the launch command to passthrough all USB devices on the USB host.
...
The section below describes setup, running and configuration of an Ubuntu VM using EMT Virt.
...
7. Run the command shown below to upgrade Ubuntu software to the latest in the guest VM.
...
12. After reboot, check that the kernel is the installed version:
   ...
   It should read: *6.6.50-lts2023-iotg*
...
15. Next, properly shut down the guest OS. The `ubuntu.qcow2` image is now ready to use.
...
2. Follow the steps in the [Creating Ubuntu VM Image](#creating-ubuntu-vm-image) section to create and setup the Ubuntu guest images. Make sure that the images are named as `ubuntu.qcow2`, `ubuntu2.qcow2`, `ubuntu3.qcow2` and `ubuntu4.qcow2`.
...
For Ubuntu guest VMs, PCI Wi-Fi device passthrough can be setup by adding the `--passthrough-pci-wifi` parameter to the guest VM launch command:
