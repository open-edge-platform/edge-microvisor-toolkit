...
install -D -m 644 %{SOURCE1} %{buildroot}/%{_libdir}/systemd/system/ephemeral-disk-warning.service
install -D -m 644 %{SOURCE2} %{buildroot}%{_sysconfdir}/ephemeral-disk-warning.conf
install -D -m 644 %{SOURCE3} %{buildroot}%{_bindir}/ephemeral-disk-warning

mkdir -p %{buildroot}%{_prefix}/lib/dracut/modules.d/97walinuxagent/
install -D -m 755 %{SOURCE4} %{buildroot}%{_prefix}/lib/dracut/modules.d/97walinuxagent/module-setup.sh
...
* Tue Nov 19 2024 Anuj Mittal <anuj.mittal@intel.com> - 2.11.1.4-4
- Fix sed to look for waagent in sbindir
