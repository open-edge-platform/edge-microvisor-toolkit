...
prepare_chroot_environment() {
  ...
  for macro_file in SPECS/edge-rpm-macros/macros* SPECS/pyproject-rpm-macros/macros.pyproject SPECS/perl/macros.perl; do
...
function specs_dir_from_spec_path {
  ...
  echo "$(realpath "$(dirname "$1")/../../SPECS")/edge-rpm-macros"
}
